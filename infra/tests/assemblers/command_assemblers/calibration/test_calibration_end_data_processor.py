...
class CalibrationEndAssemblerTest(TestCase):
    def setUp(self) -> None:
        ...
        session.user = Mock(spec=get_user_model())
        session.user.bmi = 5
        self.processor = CalibrationEndDataProcessor(session)
        self.command_data = {
            "user": "1",
    ...
    @patch(
        'infra.assemblers.command_assemblers.calibration.calibration_end_data_processor.CalibrationEndDataProcessor.get_universal_calibration_gold_standard')
    def test_GivenSessionAndProcedure_whenCalculateUserGoldStandard_thenReturnUserGoldStandard(self,
                                                                                               get_universal_calibration_gold_standard):
        dataclass_procedure_gold_standard = Mock(spec=DataClassProcedureGoldStandard,
                                                 semg=[1, 2, 3],
                                                 inertial=[1, 2, 3])
        get_universal_calibration_gold_standard.return_value = dataclass_procedure_gold_standard
        session = Mock(spec=Session)
        session.user.bmi = 5
        procedure = Mock(spec=Procedure)

        user_calibration_gs = self.processor._calculate_user_calibration_gold_standard(session, procedure)
    ...
    def test_GivenUserCalibrationGoldStandardAndMean_whenCalculateCalibrationFactor_thenReturnCalibrationFactor(self):
        ...
        calibration_factor = self.processor.calculate_calibration_factor(user_gs, user_mean)
        self.assertIsNotNone(calibration_factor)
        self.assertGreater(calibration_factor, 0)

    def test_GivenUniversalTreatmentGoldStandardAndCalibrationFactor_whenCalculate_thenReturnUserGoldStandardForTreatment(self):
        treatment_gs = DataClassTreatmentGoldStandard(semg=[10, 12, 14], inertial=[4, 6, 8])
        calibration_factor = 5

        user_treatment_gs = self.processor.calculate_user_treatment_gold_standard(calibration_factor, treatment_gs)
        self.assertIsInstance(user_treatment_gs, DataClassTreatmentGoldStandardForUser)
        self.assertEqual(user_treatment_gs.semg, [250.0, 300.0, 350.0])
        self.assertEqual(user_treatment_gs.inertial, [100.0, 150.0, 200.0])

    def test_GivenTreatmentGoldStandardRawData_whenGetTreatmentGoldStandard_thenReturnDataClassTreatmentGoldStandardFromDB(self):
        inertial_data = {
            "l5s1_lateral": "1.4",
            "l5s1_axial": "1.4",
            "l5s1_flexion": "2.4",
        ...
        actual_treatment_gs = self.processor.get_universal_treatment_gold_standard()
    ...
    def test_GivenCalibrationGoldStandard_whenGetCalibrationGoldStandard_thenReturnDataClassCalibrationGoldStandard(
        ...
        actual_gs = self.processor.get_universal_calibration_gold_standard(procedure)
